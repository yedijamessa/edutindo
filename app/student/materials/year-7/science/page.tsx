...
export default async function Year7SciencePage() {
  const years = await listCurriculumOutline();
  const year = years.find((item) => item.slug === "year-7");
  const subject = year?.subjects.find((item) => item.slug === "science");
            ...
            <section className="relative overflow-hidden rounded-3xl border border-sky-200 bg-white p-8 md:p-10 shadow-sm">
              ...
              <div className="relative">
                ...
                <p className="mt-3 text-lg max-w-3xl text-slate-600">
                  Chapter and lesson data is now loaded from Curriculum Portal and can be manually ordered there.
                </p>
            ...
            {!year || !subject ? (
              <Card>
                <CardContent className="p-6 text-sm text-muted-foreground">
                  Year 7 Science is not configured yet. Open Admin Curriculum Portal to add it.
                </CardContent>
              </Card>
            ) : (
                ...
                {subject.chapters.map((chapter, index) => (
                  ...
                  >
                    <CardHeader>
                      ...
                      <CardDescription>{chapter.weekRange || "Week range not set"}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="text-sm text-muted-foreground">
                        <span className="font-medium text-foreground">{chapter.lessonCount}</span> lessons
                      </div>
